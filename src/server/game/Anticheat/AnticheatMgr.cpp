...
void AnticheatMgr::SpeedHackDetection(Player* player,MovementInfo movementInfo)
{
	...
	if ( player->GetMapId() == 489 && !player->isDead() ){
		if (distance2D > 300 && distance2D < 3000 && (player->HasAura(23335) || player->HasAura(23333)) )
		{
			...
			sWorld->BanAccount(BAN_CHARACTER, player->GetName(), "7d", "Uso de Tele Hack em Warsong.", "AntiHack WoW-Brasil");
			std::string msg;
			msg = std::string(player->GetName()) + " foi banido por uso de Tele Hack em Warsong.";
